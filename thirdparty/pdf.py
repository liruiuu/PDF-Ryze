...
from header_and_footer import insert_header_and_footer, remove_header_and_footer
...
from scale import resize_pdf_by_dim, resize_pdf_by_paper_size, resize_pdf_by_scale
...
from watermark import (
    detect_watermark_index_helper,
    remove_watermark_by_index,
    remove_watermark_by_text,
    remove_watermark_by_type,
    watermark_pdf_by_image,
    watermark_pdf_by_pdf,
    watermark_pdf_by_text,
)
import time

logger.add(
    logpath, rotation="1 week", retention="10 days", level="DEBUG", encoding="utf-8"
)


def main_pdf(s):
    print("pdf_python开始运行!!!!!!!!!!!!!")
    parser = getParser()
    args = parser.parse_args(s)
    logger.debug(args)
    if args.which == "merge":
        merge_pdf(
            doc_path_list=args.input_path_list,
        ...
        if args.mode == "chunk":
            ...
            split_pdf_by_toc(
                doc_path=args.input_path, level=args.toc_level, output_path=args.output
            )
        ...
        if args.method == "blank":
            ...
            insert_pdf(
                doc_path1=args.input_path1,
                doc_path2=args.input_path2,
        ...
        replace_pdf(
            doc_path1=args.input_path1,
            doc_path2=args.input_path2,
            src_range=args.src_page_range,
            dst_range=args.dst_page_range,
            output_path=args.output,
        )
        ...
        change_password_pdf(
            doc_path=args.input_path,
            upw=args.old_user_password,
            new_upw=args.user_password,
            opw=args.old_owner_password,
            new_opw=args.owner_password,
            output_path=args.output,
        )
        ...
        if args.bookmark_which == "add":
            ...
            extract_toc(
                doc_path=args.input_path, format=args.format, output_path=args.output
            )
            ...
            transform_toc_file(
                toc_path=args.toc,
                level_dict_list=level_dict_list,
                delete_level_below=args.delete_level_below,
                add_offset=args.add_offset,
                default_level=args.default_level,
                is_remove_blanklines=args.remove_blank_lines,
                output_path=args.output,
            )
        ...
        if args.method == "grid":
            ...
            cut_pdf_by_breakpoints(
                doc_path=args.input_path,
                h_breakpoints=args.h_breakpoints,
                v_breakpoints=args.v_breakpoints,
                page_range=args.page_range,
                output_path=args.output,
            )
        ...
        if args.method == "bbox":
            crop_pdf_by_bbox(
                ...
                bbox=args.bbox,
                unit=args.unit,
                keep_page_size=args.keep_size,
                page_range=args.page_range,
                output_path=args.output,
            )
        elif args.method == "margin":
            crop_pdf_by_page_margin(
                doc_path=args.input_path,
                margin=args.margin,
                unit=args.unit,
                keep_page_size=args.keep_size,
        ...
        if args.watermark_which == "add":
            if args.type == "text":
                ...
                watermark_pdf_by_text(
                    ...
                    text_fill_color_rgb=color,
                    text_fill_alpha=args.opacity,
                    num_lines=args.num_lines,
                    line_spacing=args.line_spacing,
                    word_spacing=args.word_spacing,
                    multiple_mode=args.multiple_mode,
            ...
            detect_watermark_index_helper(
                doc_path=args.input_path,
                wm_page_number=args.wm_index,
                outpath=args.output,
            )
    elif args.which == "mask":
        if args.type == "rect":
            mask_pdf_by_rectangle(
                doc_path=args.input_path,
                bbox_list=args.bbox,
                color=args.color,
                opacity=args.opacity,
            ...
            mask_pdf_by_rectangle_annot(
                doc_path=args.input_path,
                annot_page=args.annot_page,
                color=args.color,
                opacity=args.opacity,
                angle=args.angle,
                page_range=args.page_range,
        ...
        if args.input_path.lower().endswith(".pdf"):
            ...
            args.input_path.lower().endswith(".png")
            or args.input_path.lower().endswith(".jpg")
            or args.input_path.lower().endswith(".jpeg")
        ):
            make_dual_layer_pdf_from_image(
                doc_path=args.input_path, lang=args.lang, output_path=args.output
            )
        else:
